...
def delve(cur):
    ...
    lowers = [e for e in cur_path if e[0].islower()]
    can_take_second = len(lowers) == len(set(lowers))

    for neighbor in transitions[cur]:
        if neighbor == 'start' or neighbor.islower() and neighbor in cur_path and not can_take_second:
            continue
        delve(neighbor)
